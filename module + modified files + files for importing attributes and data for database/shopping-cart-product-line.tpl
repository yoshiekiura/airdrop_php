	...
	<td class="cart_quantity text-center">
		...
		{else}
			...
			{if !isset($customizedDatas.$productId.$productAttributeId) OR $quantityDisplayed > 0}
				...
				{*{assign var='has_custom_qty_attribute' value=Product::hasCustomQuantityAttribute($product.id_product)}
				if $has_custom_qty_attribute
					{Combination::getDynamicPricedProductOrderQuantity($product.id_product_attribute)}{*$product|@print_r*} {* @!$ *}
				{*else*}
						<input {if (method_exists('Product', 'isDynamicallyPricedProduct') && Product::isDynamicallyPricedProduct($product.id_product)) } {* @!$ *}readonly{/if} size="2" type="text" autocomplete="off" class="cart_quantity_input form-control grey" value="{if $quantityDisplayed == 0 AND isset($customizedDatas.$productId.$productAttributeId)}{$customizedDatas.$productId.$productAttributeId|@count}{else}{$product.cart_quantity-$quantityDisplayed}{/if}"  name="quantity_{$product.id_product}_{$product.id_product_attribute}_{if $quantityDisplayed > 0}nocustom{else}0{/if}_{$product.id_address_delivery|intval}" />

				{*/if*}
				<div class="cart_quantity_button clearfix">
				{if $product.minimal_quantity < ($product.cart_quantity-$quantityDisplayed) OR $product.minimal_quantity <= 1}
					{if (method_exists('Product', 'isDynamicallyPricedProduct') && !Product::isDynamicallyPricedProduct($product.id_product)) } {* @!$ *}
